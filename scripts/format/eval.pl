...
!!! modified to adapt to CONLL08 format (by creating tmp files) !!!
...
{ # read_sent
  ...
  { # main reading loop
    ...
    {
	if(($fields_s{word} eq "<num>") && ($fields_g{word} =~ /^\d+|\d+\.\d*$/)){}
...
##--------------------------------
##creating tmp files for CONLL-X format (just copy c9c10 to c7c8)##
my $TMP_ENDING = "thiscouldnotbe";
open (GOLD08, "<$opt_g") || die "Could not open gold standard file $opt_g\n" ;
open (SYS08,  "<$opt_s") || die "Could not open system output file $opt_s\n" ;

my $opt_g_06 = $opt_g . $TMP_ENDING;
my $opt_s_06 = $opt_s . $TMP_ENDING;
open (GOLD06, ">$opt_g_06");
open (SYS06,  ">$opt_s_06");
while(<GOLD08>){
	chomp;
	my @l = split(/\t/);
	if(@l >= 10){
		print GOLD06 "$l[0]\t$l[1]\t$l[2]\t$l[4]\t$l[4]\t$l[5]\t$l[8]\t$l[9]\t$l[8]\t$l[9]\n";
	}
	else{
		print GOLD06 "$_\n";
	}
...
while(<SYS08>){
	...
	my @l = split(/\t/);
	if(@l >= 10){
		print SYS06 "$l[0]\t$l[1]\t$l[2]\t$l[4]\t$l[4]\t$l[5]\t$l[8]\t$l[9]\t$l[8]\t$l[9]\n";
	}
	else{
		print SYS06 "$_\n";
	}
...
##--------------------------------
open (GOLD, "<$opt_g_06") || die "Could not open gold standard file $opt_g\n" ;
open (SYS,  "<$opt_s_06") || die "Could not open system output file $opt_s\n" ;
